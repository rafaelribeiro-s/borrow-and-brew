...
### Step 4:
First Shadowing of the CPF (Cleaning Characters). Now, let's move on to the CPF. Redeclare the CPF variable using `let`. Use Rust methods to replace or remove the periods `.` and the hyphen `-` from the text, leaving only numbers. (Hint: you can chain the `.replace(".", "")` and `.replace("-", "")` methods).

### Step 5:
Second Shadowing of the CPF (Type Mutation - The Power of Shadowing). To prove the usefulness of Shadowing, Ana needs the CPF to become a 64-bit integer (`i64` or `u64`), as numbers occupy less space in the database than text. Redeclare the CPF variable using `let` and convert the pure numeric text from the previous step into a numeric type (Hint: use the `.parse().expect(...)` methods).

### Step 6:
The Final Delivery to Ana and João. Use `println!` to display the final result on the screen. Show the perfectly sanitized name and prove that the CPF is now an actual number by performing a simple mathematical operation with it (like adding `+ 0` or displaying its type) to ensure the compiler accepted the type mutation.